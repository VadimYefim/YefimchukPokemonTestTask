...
const useGetPokemonListData = () => {
    const getPokemonData = (
        ...
        return async (dispatch: any) => {
            await axios
                .get<any>(url || 'https://pokeapi.co/api/v2/pokemon')
                .then((response: AxiosResponse<any>) => {
                    const mappedPokemonList: PokemonItem[] =
                        getMapedPokemonList(response?.data?.results);

                    const mappedRequestData: RequestData = getMapedRequestData(
                        response?.data
                    );
